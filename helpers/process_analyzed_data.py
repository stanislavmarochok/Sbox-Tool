...
def process_data():
    csv_folder = './for_article/sboxes_datasets'
    ...
    exportHelper.exportDataframeToCsvToFolder(result_df, 'for_article', 'sboxes_datasets_processed', 'statistics.csv')
...
def get_dataframe_with_reordered_columns(df):
    ...
    for column in cols:
        if 'nonlinearity' in column:
            nonlinearity_columns.append(column)

    sorted_cols.extend(nonlinearity_columns)
...
def process_file(csv_filename, csv_folder):
    ...
    nonlinearity_column = df['nonlinearity']
    nolinearities = get_sboxes_count_with_nonlinearity(nonlinearity_column)
    for nolinearity in nolinearities.keys():
        result[f'sboxes_count_with_nonlinearity_{nolinearity}'] = nolinearities.get(f'{nolinearity}')
...
def get_sboxes_count_with_nonlinearity(nonlinearities):
    nonlinearity_result = {}
    for item in nonlinearities:
        if nonlinearity_result.get(str(item)) is None:
            nonlinearity_result[str(item)] = 0
        nonlinearity_result[str(item)] += 1
    return nonlinearity_result
